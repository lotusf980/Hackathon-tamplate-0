...
const Details = async () => {
  ...
  const res = await client.fetch(`
      *[_type == "productDetails"] | order(_createdAt asc) {
        _id, // Sanity's unique identifier
        title,
        price,
        rating,
        reviewCount,
    ...
    `);
    const {_id,title,price,rating, reviewCount, description,imageThumbnails,mainImage,socialMediaLinks,
      url,icon}
    }
  return (
    <div className="max-w-[1440px] h-auto lg:h-[820px] mx-auto px-4 lg:px-0">
      <div className="max-w-[1241.01px] h-auto lg:h-[730.87px] m-auto mt-11 bg-white flex flex-col lg:flex-row lg:justify-between">
        ...
        <div className="w-full lg:w-[606.01px] h-auto lg:h-[730.87px] mt-8 lg:mt-0">
          {/* Title and Price */}
          <div>
            <h1 className="text-[24px] lg:text-[42px]">{res.title}</h1>
            <h2 className="text-[18px] lg:text-[24px]">{res.price}</h2>
          </div>
...
const Details = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (res && res.length > 0) {
        setProduct(res[0]);  // Set first product
      }
    ...
    <div className="max-w-[1440px] h-auto lg:h-[820px] mx-auto px-4 lg:px-0">
      <div className="max-w-[1241.01px] h-auto lg:h-[730.87px] m-auto mt-11 bg-white flex flex-col lg:flex-row lg:justify-between">
        ...
        <div className="w-full lg:w-[553px] flex flex-row">
          {/* Image Thumbnails */}
          <div className="w-[76px] hidden lg:flex flex-col items-center">
            {imageThumbnails.map((src: any, idx: number) => (
        ...
        <div className="w-full lg:w-[606.01px] h-auto lg:h-[730.87px] mt-8 lg:mt-0">
          {/* Title and Price */}
          <div>
            <h1 className="text-[24px] lg:text-[42px]">{title}</h1>
            <h2 className="text-[18px] lg:text-[24px]">{price}</h2>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-14 mt-3">
            <button className="flex flex-row gap-2">
              ...
              {rating % 1 !== 0 && <FaStarHalf className="text-yellow-300" />}
            </button>
          ...
          {/* Description */}
          <div>
            <p className="text-[13px] mt-4 text-justify">{description}</p>
          </div>
